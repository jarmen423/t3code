...
const COMPACT_SLASH_COMMAND = "/compress" as const;
...
export interface HermesAdapterOptions {
  ...
  /** Runtime factory seam for tests; production uses `makeHermesAcpRuntime`. */
...
interface HermesSessionContext {
   ...
   * the adapter remembers them instead.
   */
...
/**
 * Maps a T3 decision to the option id Hermes advertised for it. Option ids are
 * opaque (`allow_session` carries kind `allow_always`), so the match is by kind
 * and the advertised id is sent back untouched. No match means the choice is
 * rejected, never redirected to an unrelated option.
 */
...
function parseHermesResumeCursor(raw: unknown): { sessionId: string } | undefined {
...
export const makeHermesAdapter = Effect.fn("makeHermesAdapter")(function* (
  ...
  const startSession: HermesAdapterShape["startSession"] = (input) =>
    withThreadLock(
      ...
      Effect.gen(function* () {
        ...
        const resumeSessionId = parseHermesResumeCursor(input.resumeCursor)?.sessionId;

        return yield* Effect.gen(function* () {
          const mcpSession = McpProviderSession.readMcpProviderSession(input.threadId);
          const makeRuntime = options.makeRuntime ?? makeHermesAcpRuntime;
          ...
          yield* applyHermesAcpMode({
            ...
            modeId: hermesPermissionMode(input.runtimeMode),
            mapError: (cause) => cause,
          });
        ...
        Effect.tapError((cause) =>
          isHermesAuthRequiredError(cause) ? (options.onAuthRequired ?? Effect.void) : Effect.void,
        ),
        Effect.mapError((cause) =>
          isAcpError(cause)
  ...
  const sendTurn: HermesAdapterShape["sendTurn"] = Effect.fn("HermesAdapter.sendTurn")(
    function* (input) {
          ...
          Effect.gen(function* () {
            ...
            if (context.stopEpoch !== observedStopEpoch) {
              return yield* new ProviderAdapterRequestError({
                provider: PROVIDER,
                method: "session/prompt",
                detail: "Hermes turn was superseded by a stop before it started.",
              });
            }
            ...
            // wait for the in-flight prompt to resolve first.
  ...
  const respondToUserInput: HermesAdapterShape["respondToUserInput"] = (threadId) =>
    Effect.fail(
      new ProviderAdapterRequestError({
        provider: PROVIDER,
        method: "session/request_permission",
        detail: `Hermes does not issue structured user-input requests (thread ${threadId}).`,
      }),
    );
    ...
    rollbackThread: (_threadId: ThreadId, _numTurns: number) =>
      Effect.fail(
        new ProviderAdapterValidationError({
          ...
          issue:
            "Hermes does not support conversation rewind over ACP. Start a new thread instead.",
        }),
      ),

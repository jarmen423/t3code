...
export const DevinSettings = makeProviderSettingsSchema(
  {
    // Off by default (like Cursor, Grok, Hermes, and OpenCode): the binding is
    // not yet stable enough to probe on every install. Users opt in from Settings.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
...
export const MuseSettings = makeProviderSettingsSchema(
  {
    // Off by default: Muse runs through the muse-acp-bridge binary, which most
    // installs will not have on PATH. Users opt in from Settings.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("muse-acp-bridge").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the Muse ACP bridge binary.",
        providerSettingsForm: { placeholder: "muse-acp-bridge", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(CustomModelSetting).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
